...
export class AdminOrdersComponent implements OnInit {
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0)
    this.http.post('http://backend-mobikart-alb-2052532502.us-east-1.elb.amazonaws.com/getAdminOrders', JSON.stringify({ 'id': 1 }), { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) }).subscribe((res) => {
      this.result = res
